...
This guide is an attendee-friendly companion to `01_quickstart_underlying_code.md`. Use it in session to explain how skills work, when to use each one, and how to avoid common mistakes.

## 1) Skill Mental Model

A skill is a reusable instruction package for the assistant. It helps the assistant choose better steps and use stable scripts instead of improvising every time.
...
-   "For filing [`1873431`](https://docquery.fec.gov/cgi-bin/forms/C00770941/1873431/), show me the summary totals only."
-   "Analyze filing [`1896830`](https://docquery.fec.gov/cgi-bin/forms/C00089482/1896830/) ([Utah GOP](https://www.fec.gov/data/committee/C00089482/)) and give me the total cash on hand."
...
Bad: - "Look at this filing."

Good: - "Analyze filing `1896830`. Start with summary-only. Then show top 10 Schedule A contributors as a table with name, state, amount, date."

Bad: - "Get weather for Denver."

Good: - "Use the weather skill workflow: resolve coordinates for Denver, CO, fetch 7-day forecast, and return both table and JSON."

Why this matters: - Better prompts specify input, workflow, output format, and limits.
...
For first-time attendees, enforce this checklist: - Always run summary/overview before full data pulls. - Limit or aggregate outputs before sharing. - Prefer script outputs over hand-typed field names. - Verify field names from reference docs. - Keep raw data out of chat when it is large; summarize instead.
